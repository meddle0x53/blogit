defmodule Blogit.Components.PostsTest do
  ...
  test "`posts` state is calculated from the posts_provider", %{pid: pid} do
    ...
    assert expected_post_names |> Enum.all?(fn name ->
  ...
  test "call `:all`, returns a list of all posts Blogit.Models.Post structs",
  %{pid: pid} do
  ...
  "posts as Blogit.Models.Post.Meta structs, sorted by creation", %{pid: pid} do
  ...
  %{pid: pid} do
    posts =
      GenServer.call(pid, {:filter, %{"author" => "meddle"}, 0, :infinity})
