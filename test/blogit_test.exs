defmodule BlogitTest do
  ...
  setup_all do
    ...
    Agent.update(Memory, fn data ->
      %{data | stored_files: Fixtures.posts(), updates: Enum.map(Fixtures.posts(), path)}
    end)
  ...
  describe ".filter_posts" do
    ...
    test "filters posts by multiple types of filters, the list is sorted " <> "with newest first" do
    ...
    test "the list of posts returns all the posts meeting the filtering " <> "criteria by default" do
    ...
    test "the `size` option can be used to change the default of " <> "`:infinity` posts returned" do
  ...
  describe "Blogit.Server" do
    test "the state of the process is all the blog data - " <> "the posts and the configuration" do
      %{
